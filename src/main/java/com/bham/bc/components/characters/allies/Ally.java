...
/**
 * <h1>Generic ally bot</h1>
 *
 * <p>This class defines all the common behavior an ally character can show. All the method calls are
 * determined by the state machine each ally has.</p>
 */
public abstract class Ally extends GameCharacter {
     ...
     * @param hp    health points the ally should have
     */
    protected Ally(double x, double y, double speed, double hp) {
    ...
    protected void search(ItemType itemType) {
        ...
        // Checks if the ally intersects the point in the list of path edges and gets the next target point if so
     ...
     * obstacle, this method will make the ally aim at that ally. So free path condition must be checked. Alternatively,
     ...
     * @return the StateMachine for that specific ally
     */
